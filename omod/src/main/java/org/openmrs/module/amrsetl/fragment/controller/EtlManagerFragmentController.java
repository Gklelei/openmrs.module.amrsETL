package org.openmrs.module.amrsetl.fragment.controller;
...
import org.openmrs.module.amrscore.etl.ETLProcedureBuilder;
...
public class EtlManagerFragmentController {
	...
	public void controller(FragmentModel model) {
		...
		final String sqlSelectQuery = "SELECT script_name, start_time, stop_time, error FROM amrsemr_etl.etl_script_status order by start_time desc limit 10;";
					...
					try {
						...
						if (resultSet != null) {
							...
							while (resultSet.next()) {
								...
								ret.add(SimpleObject.create("script_name", row[0], "start_time",
								    row[1] != null ? row[1].toString() : "", "stop_time", row[2] != null ? row[2].toString()
		...
		sf.getCurrentSession().doWork(new Work() {
			...
			public void execute(Connection connection) throws SQLException {
				try {
					...
					ResultSet rs = stmt.executeQuery("SHOW OPEN TABLES WHERE (In_use > 0 AND `Database` = 'amrsemr_etl');");
					...
					while (rs.next()) {
						...
						if (rs.getInt("In_use") > 0) {
							status.add(SimpleObject.create("process", "locked"));
						...
						try {
							...
							if (resultSet != null) {
								...
								while (resultSet.next()) {
									...
									ret.add(SimpleObject.create("script_name", row[0], "start_time",
									    row[1] != null ? row[1].toString() : "", "stop_time",
									    row[2] != null ? row[2].toString() : "", "status", row[3] != null ? "Pending"
									            : "Success"));
								}
							}
							finalTx.commit();
		...
		sf.getCurrentSession().doWork(new Work() {
			...
			public void execute(Connection connection) throws SQLException {
				try {
					Statement stmt = connection.createStatement();
					ResultSet rs = stmt
					        .executeQuery("SHOW OPEN TABLES WHERE (In_use > 0 AND `Database` LIKE 'amrsemr_etl') OR (In_use > 0 AND `Database` LIKE 'kenyaemr_datatools');");
					ResultSetMetaData metaData = rs.getMetaData();
	...
	/**
	 * Recreates tables for DWAPI It invokes stored procedures for creating DWAPI related tables
	 * 
	 * @param ui
	 * @return
	 */
		...
		sf.getCurrentSession().doWork(new Work() {
			...
			public void execute(Connection connection) throws SQLException {
				try {
					Statement stmt = connection.createStatement();
					ResultSet rs = stmt
					        .executeQuery("SHOW OPEN TABLES WHERE (In_use > 0 AND `Database` LIKE 'amrsemr_etl') OR (In_use > 0 AND `Database` LIKE 'kenyaemr_datatools') OR (In_use > 0 AND `Database` LIKE 'dwapi_etl');");
					ResultSetMetaData metaData = rs.getMetaData();
