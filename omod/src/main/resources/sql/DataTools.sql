...
-- Log start time
INSERT INTO amrs_etl.etl_script_status(script_name, start_time) VALUES('KenyaEMR_Data_Tool', NOW());
...
death_date,
voided
from amrs_etl.etl_patient_demographics;
...
voided
from amrs_etl.etl_hiv_enrollment;
...
from amrs_etl.etl_patient_hiv_followup;
...
date_voided,
dispensing_provider
from amrs_etl.etl_pharmacy_extract;
...
natural_causes,
non_natural_cause
from amrs_etl.etl_patient_program_discontinuation;
...
SELECT "Successfully created pharmacy extract table";
  ...
  create table kenyaemr_datatools.mch_enrollment as
    ...
    from amrs_etl.etl_mch_enrollment;
  ...
  create table kenyaemr_datatools.mch_antenatal_visit as
    ...
    from amrs_etl.etl_mch_antenatal_visit;
...
-- create table mch_delivery table
  create table kenyaemr_datatools.mch_delivery as
    ...
    from amrs_etl.etl_mchs_delivery;
...
-- create table mch_discharge table
  create table kenyaemr_datatools.mch_discharge as
    ...
    from amrs_etl.etl_mchs_discharge;
  ...
  create table kenyaemr_datatools.mch_postnatal_visit as
    select
      ...
      clinical_notes,
      appointment_date
    from amrs_etl.etl_mch_postnatal_visit;
...
SELECT "Successfully created post natal visit table";
  ...
  create table kenyaemr_datatools.hei_enrollment as
    select
     ...
     hiv_status_at_exit
    from amrs_etl.etl_hei_enrollment;
  ...
  create table kenyaemr_datatools.hei_follow_up_visit as
    select
      ...
      comments,
      next_appointment_date
    from amrs_etl.etl_hei_follow_up_visit;
  ...
  create table kenyaemr_datatools.hei_immunization as
    ...
    from amrs_etl.etl_hei_immunization;
  ...
  create table kenyaemr_datatools.tb_enrollment as
    ...
    from amrs_etl.etl_tb_enrollment;
...
next_appointment_date
from amrs_etl.etl_tb_follow_up_visit;
...
from amrs_etl.etl_tb_screening;
...
SELECT "Successfully created drug event table";
 ...
 -- create table art_preparation
    ...
    from amrs_etl.etl_ART_preparation;
...
SELECT "Successfully created art preparation table";
  ...
  create table kenyaemr_datatools.enhanced_adherence as
    select
      ...
      adherence_plan,
      next_appointment_date
    from amrs_etl.etl_enhanced_adherence;
...
SELECT "Successfully created enhanced adherence table";
  ...
  create table kenyaemr_datatools.triage as
    select
      ...
      voided
    from amrs_etl.etl_patient_triage;
 ...
 -- create table generalized_anxiety_disorder
  create table kenyaemr_datatools.generalized_anxiety_disorder as
    select
      ...
      voided
    from amrs_etl.etl_generalized_anxiety_disorder;
  ...
  create table kenyaemr_datatools.patient_contact as
    select
       ...
       (case consented_contact_listing when 1065 then "Yes" when 1066 then "No" when 1067 then "Unknown" else "" end) as consented_contact_listing,
        voided
    from amrs_etl.etl_patient_contact;
  ...
  create table kenyaemr_datatools.client_trace as
    select
      ...
      appointment_date,
      voided
    from amrs_etl.etl_client_trace;
...
t.hts_risk_score,
  ...
  t.remarks,
  t.voided
from amrs_etl.etl_hts_test t
inner join amrs_etl.etl_patient_demographics d on d.patient_id = t.patient_id and d.voided=0;
...
create table kenyaemr_datatools.hts_referral_and_linkage
  as select l.* from amrs_etl.etl_hts_referral_and_linkage l inner join amrs_etl.etl_patient_demographics d on d.patient_id = l.patient_id and d.voided=0;
...
create table kenyaemr_datatools.hts_referral
  as select r.* from amrs_etl.etl_hts_referral r inner join amrs_etl.etl_patient_demographics d on d.patient_id = r.patient_id and d.voided=0;

create table kenyaemr_datatools.current_in_care as select * from amrs_etl.etl_current_in_care;
...
create table kenyaemr_datatools.ipt_followup as select * from amrs_etl.etl_ipt_follow_up;
...
CREATE TABLE  kenyaemr_datatools.default_facility_info as SELECT * from amrs_etl.etl_default_facility_info;
CREATE TABLE kenyaemr_datatools.person_address as SELECT * from amrs_etl.etl_person_address;
...
date_last_modified,
voided
from amrs_etl.etl_alcohol_drug_abuse_screening;
...
create table kenyaemr_datatools.gender_based_violence as select * from amrs_etl.etl_gender_based_violence;
...
date_last_modified,
voided
from amrs_etl.etl_gbv_screening;
...
date_last_modified,
voided
from amrs_etl.etl_gbv_screening_action;
...
date_last_modified,
voided
from amrs_etl.etl_violence_reporting;
...
date_last_modified,
voided
from amrs_etl.etl_link_facility_tracking;
...
date_last_modified,
voided
from amrs_etl.etl_depression_screening;
...
date_last_modified,
voided
from amrs_etl.etl_adverse_events;
...
date_created,
date_last_modified
from amrs_etl.etl_allergy_chronic_illness;
...
date_last_modified,
voided
from amrs_etl.etl_ipt_screening;
...
when 162559 then 'ABC/DDI/LPV/r' end) as HAART_regimen,
         ...
         date_last_modified,
         voided
  from amrs_etl.etl_pre_hiv_enrollment_art;
...
select
       ...
       date_last_modified,
       voided
from amrs_etl.etl_covid19_assessment;
...
create table kenyaemr_datatools.prep_enrolment as
  select
         ...
         buddy_alt_phone,
         voided
  from amrs_etl.etl_prep_enrolment;
...
select
    ...
    remarks,
    voided
from amrs_etl.etl_prep_monthly_refill;
...
create table kenyaemr_datatools.cervical_cancer_screening as
  select
      ...
      clinical_notes,
      voided
  from amrs_etl.etl_cervical_cancer_screening;
...
select
  ...
  buddy_phone_number,
  voided
 from amrs_etl.etl_client_enrollment;
...
-- mat_results,
    ...
    appointment_date,
    voided
 from amrs_etl.etl_clinical_visit;
...
select
    ...
    remarks,
    voided
 from amrs_etl.etl_peer_calendar;
...
select
  uuid,
	...
	appointment_date,
	voided
 from amrs_etl.etl_sti_treatment;
...
select
  ...
  date_last_modified,
  voided
 from amrs_etl.etl_peer_tracking;
...
select
  ...
  date_last_modified,
  voided
 from amrs_etl.etl_treatment_verification;
...
select
    ...
    date_last_modified,
    voided
 from amrs_etl.etl_vmmc_enrolment;
...
select
    ...
    date_last_modified,
    voided
from amrs_etl.etl_vmmc_circumcision_procedure;
  ...
  create table kenyaemr_datatools.vmmc_client_followup as
    select
      ...
      date_last_modified,
      voided
    from amrs_etl.etl_vmmc_client_followup;
...
select
    ...
    date_last_modified,
    voided
from amrs_etl.etl_vmmc_medical_history;
...
select
    ...
    date_last_modified,
    voided
from amrs_etl.etl_vmmc_post_operation_assessment;
...
SELECT "Successfully created vmmc_post_operation_assessment table";
  ...
  create table kenyaemr_datatools.hts_eligibility_screening as
    select
      ...
      date_last_modified,
      voided
    from amrs_etl.etl_hts_eligibility_screening;
...
create table kenyaemr_datatools.drug_order as select * from amrs_etl.etl_drug_order;
...
select
    ...
    date_created,
    voided
from amrs_etl.etl_preventive_services;
...
select
    ...
    date_last_modified,
    voided
from amrs_etl.etl_overdose_reporting;
...
select uuid,
       ...
       date_created,
       date_last_modified
from amrs_etl.etl_ccc_defaulter_tracing f;
...
select patient_id,
       ...
       date_last_modified,
       voided
from amrs_etl.etl_art_fast_track;
...
select  patient_id,
     ...
     this_facility,
     voided
from amrs_etl.etl_clinical_encounter;
...
UPDATE amrs_etl.etl_script_status SET stop_time=NOW() where id= script_id;
