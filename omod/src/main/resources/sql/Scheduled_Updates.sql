...
CREATE PROCEDURE sp_update_etl_ipt_followup(IN last_update_time DATETIME)
	BEGIN
		...
		INSERT INTO amrs_etl.etl_ipt_followup(
			uuid,
			patient_id,
			visit_id,
			visit_date,
...
CREATE PROCEDURE sp_scheduled_updates()
  BEGIN
    ...
    SELECT max(start_time) into last_update_time from amrs_etl.etl_script_status where stop_time is not null;

    INSERT INTO amrs_etl.etl_script_status(script_name, start_time) VALUES('scheduled_updates', NOW());
    ...
    UPDATE amrs_etl.etl_script_status SET stop_time=NOW() where  id= update_script_id;
    DELETE FROM amrs_etl.etl_script_status where script_name in ("KenyaEMR_Data_Tool", "scheduled_updates") and start_time < DATE_SUB(NOW(), INTERVAL 12 HOUR);
