...
-- Log start time
INSERT INTO amrsemr_etl.etl_script_status(script_name, start_time) VALUES('AMRS_Data_Tool', NOW());
SET script_id = LAST_INSERT_ID();

drop database if exists amrsemr_datatools;
create database amrsemr_datatools DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;

-- -------------------------- creating patient demographics --------------------------------------
-- populate patient_demographics table
create table amrsemr_datatools.patient_demographics as
select 
patient_id,
uuid,
given_name,
...
death_date,
voided
from amrsemr_etl.etl_patient_demographics;

-- ADD INDICES

ALTER TABLE amrsemr_datatools.patient_demographics ADD PRIMARY KEY(patient_id);
ALTER TABLE amrsemr_datatools.patient_demographics ADD INDEX(Gender);
SELECT "Successfully created demographics table";

-- --------------------------- populate patient_hiv_enrollment table ---------------------------------------------
create table amrsemr_datatools.hiv_enrollment as
select 
patient_id,
uuid,
visit_id,
...
voided
from amrsemr_etl.etl_hiv_enrollment;


ALTER TABLE amrsemr_datatools.hiv_enrollment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

ALTER TABLE amrsemr_datatools.hiv_enrollment ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.hiv_enrollment ADD INDEX(arv_status);
ALTER TABLE amrsemr_datatools.hiv_enrollment ADD INDEX(date_confirmed_hiv_positive);
ALTER TABLE amrsemr_datatools.hiv_enrollment ADD INDEX(entry_point);

SELECT "Successfully created hiv enrollment table";

-- ----------------------------------- create table hiv_followup ----------------------------------------------
create table amrsemr_datatools.hiv_followup as
select
uuid,
patient_id,
visit_id,
...
from amrsemr_etl.etl_patient_hiv_followup;

ALTER TABLE amrsemr_datatools.hiv_followup ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(pregnancy_status);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(breastfeeding);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(family_planning_status);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(tb_status);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(ctx_dispensed);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(population_type);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(on_anti_tb_drugs);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(stability);
ALTER TABLE amrsemr_datatools.hiv_followup ADD INDEX(differentiated_care);

SELECT "Successfully created hiv followup table";

-- -------------------------------- create table laboratory_extract ------------------------------------------
create table amrsemr_datatools.laboratory_extract as
select
    uuid,
    encounter_id,
    patient_id,
    ...
    date_created,
    created_by
from amrsemr_etl.etl_laboratory_extract;

ALTER TABLE amrsemr_datatools.laboratory_extract ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

ALTER TABLE amrsemr_datatools.laboratory_extract ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.laboratory_extract ADD INDEX(lab_test);
ALTER TABLE amrsemr_datatools.laboratory_extract ADD INDEX(test_result);

SELECT "Successfully created lab extract table";

-- create table pharmacy_extract
create table amrsemr_datatools.pharmacy_extract as
select 
patient_id,
uuid,
visit_date,
...
date_voided,
dispensing_provider
from amrsemr_etl.etl_pharmacy_extract;

ALTER TABLE amrsemr_datatools.pharmacy_extract ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

-- create table patient_program_discontinuation
create table amrsemr_datatools.patient_program_discontinuation as
select 
patient_id,
uuid,
visit_id,
...
natural_causes,
non_natural_cause
from amrsemr_etl.etl_patient_program_discontinuation;

ALTER TABLE amrsemr_datatools.patient_program_discontinuation ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

ALTER TABLE amrsemr_datatools.patient_program_discontinuation ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.patient_program_discontinuation ADD INDEX(discontinuation_reason);
SELECT "Successfully created pharmacy extract table";

  -- create table mch_enrollment
  create table amrsemr_datatools.mch_enrollment as
    select
      patient_id,
      uuid,
      visit_id,
    ...
    from amrsemr_etl.etl_mch_enrollment;

  ALTER TABLE amrsemr_datatools.mch_enrollment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.mch_enrollment ADD INDEX(visit_date);
  SELECT "Successfully created mch enrollment table";

  -- create table mch_antenatal_visit
  create table amrsemr_datatools.mch_antenatal_visit as
    select
      patient_id,
      uuid,
      visit_id,
    ...
    from amrsemr_etl.etl_mch_antenatal_visit;

  ALTER TABLE amrsemr_datatools.mch_antenatal_visit ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.mch_antenatal_visit ADD INDEX(visit_date);
  SELECT "Successfully created mch antenatal visit table";

-- create table mch_delivery table
  create table amrsemr_datatools.mch_delivery as
    select
      patient_id,
      uuid,
      provider,
    ...
    from amrsemr_etl.etl_mchs_delivery;

  ALTER TABLE amrsemr_datatools.mch_delivery ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.mch_delivery ADD INDEX(visit_date);
  SELECT "Successfully created mchs delivery table";

-- create table mch_discharge table
  create table amrsemr_datatools.mch_discharge as
    select
      patient_id,
      uuid,
      provider,
    ...
    from amrsemr_etl.etl_mchs_discharge;
  ALTER TABLE amrsemr_datatools.mch_discharge ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.mch_discharge ADD INDEX(visit_date);

  SELECT "Successfully created mch_discharge table";

  -- create table mch_postnatal_visit
  create table amrsemr_datatools.mch_postnatal_visit as
    select
      patient_id,
      uuid,
      visit_id,
      ...
      clinical_notes,
      appointment_date
    from amrsemr_etl.etl_mch_postnatal_visit;

  ALTER TABLE amrsemr_datatools.mch_postnatal_visit ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.mch_postnatal_visit ADD INDEX(visit_date);
SELECT "Successfully created post natal visit table";

  -- create table hei_enrollment
  create table amrsemr_datatools.hei_enrollment as
    select
      serial_no,
      patient_id,
      uuid,
     ...
     hiv_status_at_exit
    from amrsemr_etl.etl_hei_enrollment;

  ALTER TABLE amrsemr_datatools.hei_enrollment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.hei_enrollment ADD INDEX(visit_date);
  SELECT "Successfully created hei_enrollment";

  -- create table hei_follow_up_visit
  create table amrsemr_datatools.hei_follow_up_visit as
    select
      patient_id,
      uuid,
      provider,
      ...
      comments,
      next_appointment_date
    from amrsemr_etl.etl_hei_follow_up_visit;

  ALTER TABLE amrsemr_datatools.hei_follow_up_visit ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

  ALTER TABLE amrsemr_datatools.hei_follow_up_visit ADD INDEX(visit_date);
  ALTER TABLE amrsemr_datatools.hei_follow_up_visit ADD INDEX(infant_feeding);

  SELECT "Successfully created hei_follow_up_visit";

  -- create table hei_immunization
  create table amrsemr_datatools.hei_immunization as
    select
      patient_id,
			visit_date,
			created_by,
    ...
    from amrsemr_etl.etl_hei_immunization;

  ALTER TABLE amrsemr_datatools.hei_immunization ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  SELECT "Successfully created hei_immunization table";


  -- create table tb_enrollment
  create table amrsemr_datatools.tb_enrollment as
    select
      patient_id,
      uuid,
      provider,
    ...
    from amrsemr_etl.etl_tb_enrollment;

  ALTER TABLE amrsemr_datatools.tb_enrollment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

  ALTER TABLE amrsemr_datatools.tb_enrollment ADD INDEX(visit_date);
SELECT "Successfully created tb enrollment table";

-- create table tb_follow_up_visit
create table amrsemr_datatools.tb_follow_up_visit as
select 
patient_id,
uuid,
provider,
...
next_appointment_date
from amrsemr_etl.etl_tb_follow_up_visit;
 
ALTER TABLE amrsemr_datatools.tb_follow_up_visit ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

ALTER TABLE amrsemr_datatools.tb_follow_up_visit ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.tb_follow_up_visit ADD INDEX(hiv_status);
SELECT "Successfully created tb followup table";


-- create table tb_screening
create table amrsemr_datatools.tb_screening as
select
patient_id,
uuid,
provider,
...
from amrsemr_etl.etl_tb_screening;

ALTER TABLE amrsemr_datatools.tb_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.tb_screening ADD INDEX(visit_date);
SELECT "Successfully created tb screening table";

 -- Table Datatools drug event
create table amrsemr_datatools.drug_event as
    select
      uuid,
      patient_id,
      date_started,
    ...
    from amrsemr_etl.etl_drug_event;

ALTER TABLE amrsemr_datatools.drug_event add FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

SELECT "Successfully created drug event table";


 -- create table art_preparation
     create table amrsemr_datatools.art_preparation as
      select
     uuid,
     patient_id,
     visit_id,
    ...
    from amrsemr_etl.etl_ART_preparation;

    ALTER TABLE amrsemr_datatools.art_preparation ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
    ALTER TABLE amrsemr_datatools.art_preparation ADD INDEX(visit_date);
    ALTER TABLE amrsemr_datatools.art_preparation ADD INDEX(ready_to_start_art);

SELECT "Successfully created art preparation table";

  -- create table enhanced_adherence
  create table amrsemr_datatools.enhanced_adherence as
    select
      uuid,
      patient_id,
      visit_id,
      ...
      adherence_plan,
      next_appointment_date
    from amrsemr_etl.etl_enhanced_adherence;
  ALTER TABLE amrsemr_datatools.enhanced_adherence ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.enhanced_adherence ADD INDEX(visit_date);

SELECT "Successfully created enhanced adherence table";


  -- create table triage
  create table amrsemr_datatools.triage as
    select
      uuid,
      patient_id,
      visit_id,
      ...
      voided
    from amrsemr_etl.etl_patient_triage;

  ALTER TABLE amrsemr_datatools.triage ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.triage ADD INDEX(visit_date);
  SELECT "Successfully created triage table";

 -- create table generalized_anxiety_disorder
  create table amrsemr_datatools.generalized_anxiety_disorder as
    select
      uuid,
      patient_id,
      visit_id,
      ...
      voided
    from amrsemr_etl.etl_generalized_anxiety_disorder;

  ALTER TABLE amrsemr_datatools.generalized_anxiety_disorder ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.generalized_anxiety_disorder ADD INDEX(visit_date);
  SELECT "Successfully created generalized anxiety disorder table";


  -- create table datatools_patient_contact
  create table amrsemr_datatools.patient_contact as
    select
        id,
        uuid,
        date_created,
       ...
       (case consented_contact_listing when 1065 then "Yes" when 1066 then "No" when 1067 then "Unknown" else "" end) as consented_contact_listing,
        voided
    from amrsemr_etl.etl_patient_contact;
  ALTER TABLE amrsemr_datatools.patient_contact ADD PRIMARY KEY(id);
  ALTER TABLE amrsemr_datatools.patient_contact ADD FOREIGN KEY (patient_related_to) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.patient_contact ADD INDEX(date_created);
  SELECT "Successfully created patient_contact table";

    -- create table datatools_client_trace
  create table amrsemr_datatools.client_trace as
    select
      id,
      uuid,
      date_created,
      ...
      appointment_date,
      voided
    from amrsemr_etl.etl_client_trace;
  ALTER TABLE amrsemr_datatools.client_trace ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_contact(id);
  ALTER TABLE amrsemr_datatools.client_trace ADD INDEX(date_created);
  SELECT "Successfully created client_trace table";

SELECT "creating hts_test table";
create table amrsemr_datatools.hts_test
  as select
  t.patient_id,
  t.visit_id,
  t.encounter_id,
...
t.hts_risk_score,
  ...
  t.remarks,
  t.voided
from amrsemr_etl.etl_hts_test t
inner join amrsemr_etl.etl_patient_demographics d on d.patient_id = t.patient_id and d.voided=0;
ALTER TABLE amrsemr_datatools.hts_test ADD FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.hts_test ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.hts_test ADD index(population_type);
ALTER TABLE amrsemr_datatools.hts_test ADD index(final_test_result);

SELECT "Successfully created hts_test table";

create table amrsemr_datatools.hts_referral_and_linkage
  as select l.* from amrsemr_etl.etl_hts_referral_and_linkage l inner join amrsemr_etl.etl_patient_demographics d on d.patient_id = l.patient_id and d.voided=0;
ALTER TABLE amrsemr_datatools.hts_referral_and_linkage ADD FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.hts_referral_and_linkage ADD index(visit_date);

SELECT "Successfully created referral_and_linkage table";

create table amrsemr_datatools.hts_referral
  as select r.* from amrsemr_etl.etl_hts_referral r inner join amrsemr_etl.etl_patient_demographics d on d.patient_id = r.patient_id and d.voided=0;

create table amrsemr_datatools.current_in_care as select * from amrsemr_etl.etl_current_in_care;
ALTER TABLE amrsemr_datatools.current_in_care add FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

create table amrsemr_datatools.ipt_followup as select * from amrsemr_etl.etl_ipt_follow_up;
alter table amrsemr_datatools.ipt_followup add FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

CREATE TABLE  amrsemr_datatools.default_facility_info as SELECT * from amrsemr_etl.etl_default_facility_info;
CREATE TABLE amrsemr_datatools.person_address as SELECT * from amrsemr_etl.etl_person_address;


  -- create table alcohol_drug_abuse_screening
create table amrsemr_datatools.alcohol_drug_abuse_screening as
select
patient_id,
uuid,
provider,
...
date_last_modified,
voided
from amrsemr_etl.etl_alcohol_drug_abuse_screening;

ALTER TABLE amrsemr_datatools.alcohol_drug_abuse_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.alcohol_drug_abuse_screening ADD INDEX(visit_date);
SELECT "Successfully created alcohol_drug_abuse_screening table";

/*Form collecting data has been discontinued
create table amrsemr_datatools.gender_based_violence as select * from amrsemr_etl.etl_gender_based_violence;
alter table amrsemr_datatools.gender_based_violence add FOREIGN KEY(client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.gender_based_violence ADD INDEX(visit_date);*/

-- create table gbv_screening
create table amrsemr_datatools.gbv_screening as
select
patient_id,
uuid,
provider,
...
date_last_modified,
voided
from amrsemr_etl.etl_gbv_screening;

ALTER TABLE amrsemr_datatools.gbv_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.gbv_screening ADD INDEX(visit_date);
SELECT "Successfully created gbv_screening table";

-- create table gbv_screening
create table amrsemr_datatools.gbv_screening_action as
select
patient_id,
uuid,
encounter_id,
...
date_last_modified,
voided
from amrsemr_etl.etl_gbv_screening_action;

ALTER TABLE amrsemr_datatools.gbv_screening_action ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.gbv_screening_action ADD INDEX(visit_date);
SELECT "Successfully created gbv_screening_action table";



-- create table etl_violence_reporting
create table amrsemr_datatools.violence_reporting as
select
uuid,
provider,
patient_id,
...
date_last_modified,
voided
from amrsemr_etl.etl_violence_reporting;

ALTER TABLE amrsemr_datatools.violence_reporting ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.violence_reporting ADD INDEX(visit_date);
SELECT "Successfully created etl_violence_reporting table";

-- create table link_facility_tracking
create table amrsemr_datatools.link_facility_tracking as
select
uuid,
provider,
patient_id,
...
date_last_modified,
voided
from amrsemr_etl.etl_link_facility_tracking;

ALTER TABLE amrsemr_datatools.link_facility_tracking ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.link_facility_tracking ADD INDEX(visit_date);
SELECT "Successfully created etl_link_facility_tracking table";

-- create table depression_screening
create table amrsemr_datatools.depression_screening as
select
patient_id,
uuid,
provider,
...
date_last_modified,
voided
from amrsemr_etl.etl_depression_screening;

ALTER TABLE amrsemr_datatools.depression_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.depression_screening ADD INDEX(visit_date);
SELECT "Successfully created depression_screening table";

-- create table adverse_events
create table amrsemr_datatools.adverse_events as
select
uuid,
provider,
patient_id,
...
date_last_modified,
voided
from amrsemr_etl.etl_adverse_events;

ALTER TABLE amrsemr_datatools.adverse_events ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.adverse_events ADD INDEX(visit_date);
SELECT "Successfully created adverse_events table";

-- create table allergies_chronic_illnesses
create table amrsemr_datatools.allergy_chronic_illness as
select
uuid,
provider,
patient_id,
...
date_created,
date_last_modified
from amrsemr_etl.etl_allergy_chronic_illness;

ALTER TABLE amrsemr_datatools.allergy_chronic_illness ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.allergy_chronic_illness ADD INDEX(visit_date);
SELECT "Successfully created allergy_chronic_illness table";

-- create table ipt_screening
create table amrsemr_datatools.ipt_screening as
select
uuid,
provider,
patient_id,
...
date_last_modified,
voided
from amrsemr_etl.etl_ipt_screening;

ALTER TABLE amrsemr_datatools.ipt_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.ipt_screening ADD INDEX(visit_date);
SELECT "Successfully created ipt_screening table";

-- create table pre_hiv_enrollment_ART
create table amrsemr_datatools.pre_hiv_enrollment_art as
  select
         uuid,
         provider,
         patient_id,
...
when 162559 then 'ABC/DDI/LPV/r' end) as HAART_regimen,
         ...
         date_last_modified,
         voided
  from amrsemr_etl.etl_pre_hiv_enrollment_art;

ALTER TABLE amrsemr_datatools.pre_hiv_enrollment_art ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.pre_hiv_enrollment_art ADD INDEX(visit_date);
SELECT "Successfully created pre_hiv_enrollment_art table";

-- create table covid_19_assessment
create table amrsemr_datatools.covid_19_assessment as
select
       uuid,
       provider,
       patient_id,
       ...
       date_last_modified,
       voided
from amrsemr_etl.etl_covid19_assessment;

ALTER TABLE amrsemr_datatools.covid_19_assessment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.covid_19_assessment ADD INDEX(visit_date);
SELECT "Successfully created covid_19_assessment table";

-- Create table prep_enrolment
create table amrsemr_datatools.prep_enrolment as
  select
         uuid,
         provider,
         patient_id,
         ...
         buddy_alt_phone,
         voided
  from amrsemr_etl.etl_prep_enrolment;

ALTER TABLE amrsemr_datatools.prep_enrolment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.prep_enrolment ADD INDEX(visit_date);
SELECT "Successfully created prep_enrolment table";

-- Create table prep_monthly_refill
create table amrsemr_datatools.prep_monthly_refill as
select
    uuid,
    provider,
    patient_id,
    ...
    remarks,
    voided
from amrsemr_etl.etl_prep_monthly_refill;

ALTER TABLE amrsemr_datatools.prep_monthly_refill ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.prep_monthly_refill ADD INDEX(visit_date);
SELECT "Successfully created prep_monthly_refill table";

-- Create table cervical_cancer_screening
create table amrsemr_datatools.cervical_cancer_screening as
  select
      uuid,
      encounter_id,
      encounter_provider,
      ...
      clinical_notes,
      voided
  from amrsemr_etl.etl_cervical_cancer_screening;

ALTER TABLE amrsemr_datatools.cervical_cancer_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.cervical_cancer_screening ADD INDEX(visit_date);
SELECT "Successfully created cervical_cancer_screening table";

-- Create table contact
create table amrsemr_datatools.kp_contact as
select
    uuid,
    client_id,
    visit_id,
    ...
    voided
 from amrsemr_etl.etl_contact;

ALTER TABLE amrsemr_datatools.kp_contact ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_contact ADD INDEX(visit_date);
SELECT "Successfully created kp_contact table";

-- Create table kp_client_enrollment
create table amrsemr_datatools.kp_client_enrollment as
select
  uuid,
  client_id,
  visit_id,
  ...
  buddy_phone_number,
  voided
 from amrsemr_etl.etl_client_enrollment;

ALTER TABLE amrsemr_datatools.kp_client_enrollment ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_client_enrollment ADD INDEX(visit_date);
SELECT "Successfully created kp_client_enrollment table";

-- Create table kp_clinical_visit
create table amrsemr_datatools.kp_clinical_visit as
select
    uuid,
    client_id,
    visit_id,
...
-- mat_results,
    ...
    appointment_date,
    voided
 from amrsemr_etl.etl_clinical_visit;

ALTER TABLE amrsemr_datatools.kp_clinical_visit ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_clinical_visit ADD INDEX(visit_date);
SELECT "Successfully created kp_clinical_visit table";

-- Create table kp_peer_calendar
create table amrsemr_datatools.kp_peer_calendar as
select
    uuid,
    client_id,
    visit_id,
    ...
    remarks,
    voided
 from amrsemr_etl.etl_peer_calendar;

ALTER TABLE amrsemr_datatools.kp_peer_calendar ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_peer_calendar ADD INDEX(visit_date);
SELECT "Successfully created kp_peer_calendar table";

-- Create table kp_sti_treatment
create table amrsemr_datatools.kp_sti_treatment as
select
  uuid,
	client_id,
	visit_id,
	...
	appointment_date,
	voided
 from amrsemr_etl.etl_sti_treatment;

ALTER TABLE amrsemr_datatools.kp_sti_treatment ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_sti_treatment ADD INDEX(visit_date);
SELECT "Successfully created kp_sti_treatment table";

-- Create table kp_peer_tracking
create table amrsemr_datatools.kp_peer_tracking as
select
  uuid,
  provider,
  client_id,
  ...
  date_last_modified,
  voided
 from amrsemr_etl.etl_peer_tracking;

ALTER TABLE amrsemr_datatools.kp_peer_tracking ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_peer_tracking ADD INDEX(visit_date);
SELECT "Successfully created kp_peer_tracking table";

-- Create table kp_treatment_verification
create table amrsemr_datatools.kp_treatment_verification as
select
  uuid,
  provider,
  client_id,
  ...
  date_last_modified,
  voided
 from amrsemr_etl.etl_treatment_verification;

ALTER TABLE amrsemr_datatools.kp_treatment_verification ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.kp_treatment_verification ADD INDEX(visit_date);
SELECT "Successfully created kp_treatment_verification table";

-- Create table vmmc_enrolment
create table amrsemr_datatools.vmmc_enrolment as
select
    uuid,
    provider,
    patient_id,
    ...
    date_last_modified,
    voided
 from amrsemr_etl.etl_vmmc_enrolment;

ALTER TABLE amrsemr_datatools.vmmc_enrolment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.vmmc_enrolment ADD INDEX(visit_date);
SELECT "Successfully created vmmc_enrolment table";

-- Create table vmmc_enrolment
create table amrsemr_datatools.vmmc_circumcision_procedure as
select
    uuid,
    provider,
    patient_id,
    ...
    date_last_modified,
    voided
from amrsemr_etl.etl_vmmc_circumcision_procedure;

  -- Create table vmmc_client_followup
  create table amrsemr_datatools.vmmc_client_followup as
    select
      uuid,
      provider,
      patient_id,
      ...
      date_last_modified,
      voided
    from amrsemr_etl.etl_vmmc_client_followup;

ALTER TABLE amrsemr_datatools.vmmc_circumcision_procedure ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.vmmc_circumcision_procedure ADD INDEX(visit_date);
SELECT "Successfully created vmmc_circumcision_procedure table";

-- Create table vmmc_medical_history
create table amrsemr_datatools.vmmc_medical_history as
select
    uuid,
    provider,
    patient_id,
    ...
    date_last_modified,
    voided
from amrsemr_etl.etl_vmmc_medical_history;

ALTER TABLE amrsemr_datatools.vmmc_medical_history ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.vmmc_medical_history ADD INDEX(visit_date);
SELECT "Successfully created vmmc_medical_history table";

-- Create table vmmc_post_operation_assessment

create table amrsemr_datatools.vmmc_post_operation_assessment as
select
    provider,
    patient_id,
    visit_id,
    ...
    date_last_modified,
    voided
from amrsemr_etl.etl_vmmc_post_operation_assessment;

ALTER TABLE amrsemr_datatools.vmmc_post_operation_assessment ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.vmmc_post_operation_assessment ADD INDEX(visit_date);
SELECT "Successfully created vmmc_post_operation_assessment table";

  -- --------------------------- populate hts_eligibility screening table ---------------------------------------------
  create table amrsemr_datatools.hts_eligibility_screening as
    select
      patient_id,
      visit_id,
      encounter_id,
      ...
      date_last_modified,
      voided
    from amrsemr_etl.etl_hts_eligibility_screening;

  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(patient_id);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(visit_id);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(visit_date);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(department);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(population_type);
  ALTER TABLE amrsemr_datatools.hts_eligibility_screening ADD INDEX(eligible_for_test);

-- Creating drug_orders table --
create table amrsemr_datatools.drug_order as select * from amrsemr_etl.etl_drug_order;
ALTER TABLE amrsemr_datatools.drug_order add FOREIGN KEY(patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);

-- create table preventive_services
create table amrsemr_datatools.preventive_services as
select
    patient_id,
    visit_date,
    provider,
    ...
    date_created,
    voided
from amrsemr_etl.etl_preventive_services;

ALTER TABLE amrsemr_datatools.preventive_services ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.preventive_services ADD INDEX(patient_id);
ALTER TABLE amrsemr_datatools.preventive_services ADD INDEX(visit_date);
SELECT "Successfully created preventive_services table";

-- create table overdose_reporting
create table amrsemr_datatools.overdose_reporting as
select
    client_id,
    visit_id,
    encounter_id,
    ...
    date_last_modified,
    voided
from amrsemr_etl.etl_overdose_reporting;
ALTER TABLE amrsemr_datatools.overdose_reporting ADD FOREIGN KEY (client_id) REFERENCES amrsemr_datatools.patient_demographics(patient_id);
ALTER TABLE amrsemr_datatools.overdose_reporting ADD INDEX(client_id);
ALTER TABLE amrsemr_datatools.overdose_reporting ADD INDEX(visit_date);
ALTER TABLE amrsemr_datatools.overdose_reporting ADD INDEX(naloxone_provided);
ALTER TABLE amrsemr_datatools.overdose_reporting ADD INDEX(outcome);
SELECT "Successfully created overdose_reporting table";

-- Create table ccc_defaulter_tracing
create table amrsemr_datatools.ccc_defaulter_tracing as
select uuid,
       provider,
       patient_id,
       visit_id,
       ...
       date_created,
       date_last_modified
from amrsemr_etl.etl_ccc_defaulter_tracing f;
Alter table amrsemr_datatools.ccc_defaulter_tracing
    ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics (patient_id);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (visit_date);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (patient_id);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (missed_appointment_date);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (true_status);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (cause_of_death);
ALTER TABLE amrsemr_datatools.ccc_defaulter_tracing ADD INDEX (tracing_type);
SELECT "Successfully created ccc_defaulter_tracing table";

-- Create table ART fast track
create table amrsemr_datatools.art_fast_track as
select patient_id,
       visit_date,
       provider,
       location_id,
       ...
       date_last_modified,
       voided
from amrsemr_etl.etl_art_fast_track;
ALTER TABLE amrsemr_datatools.art_fast_track
    ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics (patient_id);
ALTER TABLE amrsemr_datatools.art_fast_track
...
create table amrsemr_datatools.clinical_encounter as
select  patient_id,
     visit_id,
     encounter_id,
     location_id,
     ...
     this_facility,
     voided
from amrsemr_etl.etl_clinical_encounter;
ALTER TABLE amrsemr_datatools.clinical_encounter
    ADD FOREIGN KEY (patient_id) REFERENCES amrsemr_datatools.patient_demographics (patient_id);
ALTER TABLE amrsemr_datatools.clinical_encounter
    ADD INDEX (patient_id);
ALTER TABLE amrsemr_datatools.clinical_encounter
...
UPDATE amrsemr_etl.etl_script_status SET stop_time=NOW() where id= script_id;
